function salvarInformacoes(event,formId) {
  ...
  const existingFinalJson = JSON.parse(localStorage.getItem('Infos_empresa')) || {};
  ...
  localStorage.setItem('Infos_empresa', JSON.stringify(finalJson));

  // Verificando qual é a próxima página
  let nextPage = formId.split('-');
  nextPage = `../cadastroempresa/cadastroempresa${+nextPage[1] + 1}.html`;
  console.log(nextPage);
  // Redirecionando para a próxima página usando window.location.href
  if( +formId.split('-')[1] ==5){
    localStorage.setItem('auth',JSON.stringify(true))
    localStorage.setItem('type',JSON.stringify('empresa'))
